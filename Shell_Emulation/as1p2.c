...
void refreshJobList()
{
    ...
    //perform init for pointers by setting them to head_job
    current_job = head_job;
    prev_job = head_job;
    ...
    while (current_job != NULL)
    {
        ...
        // if == 0 that means the job is still executing therefore leave it in the linked list of jobs (keep looping through)
        if (ret_pid == 0)
        {
            prev_job = current_job;
            current_job = current_job->next;
        }
        //if it is done executing, delete the job
        else
        {
            //if job to be deleted is the head job (beg of LL)
...
void listAllJobs()
{
    //does not make sense to have these (current_job is a global variable and ret_pid is not used until inside refreshJobList where it is declared)
    //struct node *current_job;
    //int ret_pid;
...
int wordCount(char *filename, char *flag)
{
    ...
    //if it does not exist --> will output -1
    int fileExist = open(filename, O_WRONLY);

    //if file does not exist then print the following
    if (fileExist < 0)
    {
    ...
    //if wc -(any other character) --> output the following
    else
    {
        printf("Unrecognized Flag\n");
    }

    //print the number of words or lines or 0 if error
    printf("%d", cnt);
    return cnt;
}
...
int main(void)
{
    ...
    while (1)
    {
        ...
        else if (!strcmp("fg", args[0]))
        {
            ...
            printf("%s%s%s", "brought job number ", args[1], " to foreground");
            waitforjob(args[1]);
        }
        else if (!strcmp("cd", args[0]))
        {
            ...
            else
            {
                ...
                int foundDirectory = chdir(args[1]);
        ...
        {
            //get size of biggest path and create an array of that length
        ...
        else
        {
            ...
            else
            {
                ...
                if (isred == 1)
                {
                    ...
                    // copy stdout to next available slot/handle
                    int outCopy = dup(1);

                    // make slot/handle 1 available
                    close(1);

                    //copy file descriptor of file to slot/handle 1 
                    dup2(fd1, 1);
